    ...
    for _ in range(1000):
        ...
        for i, node1 in enumerate(nodes):
            for j, node2 in enumerate(nodes):
                ...
                if min_distance is None or distance < min_distance:
                    ...
                    b = j
        print(
            f"Closest together are: {nodes[a]} and {nodes[b]} with distance: {min_distance}"
        )
        ...
        new_circuit = set(
            itertools.chain.from_iterable(
                [circuit for circuit in circuits if a in circuit or b in circuit]
            )
        )
        circuits = [
            circuit for circuit in circuits if a not in circuit and b not in circuit
        ] + [new_circuit]

    print(circuits)
    counts = [
        len(circuit)
        for circuit in sorted(circuits, key=lambda circuit: len(circuit))[-3:]
    ]
...
def part2(puzzle_input):
    ...
    nodes = [tuple(int(val) for val in line.split(",")) for line in puzzle_input]
    node_combos = [
        (i, k) for i in range(len(nodes)) for k in range(len(nodes)) if i != k
    ]
    sorted_combos = sorted(
        node_combos, key=lambda combo: _distance(nodes[combo[0]], nodes[combo[1]])
    )
    connected = set()
