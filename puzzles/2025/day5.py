    ...
    while ranges:
        ...
        if not consolidated_ranges:
            ...
            for consolidated_range in consolidated_ranges:
                if (
                    consolidated_range[0] <= r[0] <= consolidated_range[1]
                    or consolidated_range[0] <= r[1] <= consolidated_range[1]
                    or r[0] <= consolidated_range[0] <= r[1]
                    or r[0] <= consolidated_range[1] <= r[1]
                ):
                    consolidated_ranges.remove(consolidated_range)
                    ranges.append(
                        (
                            min(consolidated_range[0], r[0]),
                            max(consolidated_range[1], r[1]),
                        )
                    )
